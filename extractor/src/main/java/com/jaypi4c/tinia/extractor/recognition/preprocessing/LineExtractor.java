...
public class LineExtractor {
    ...
    public List<Line2D> execute(BufferedImage image, String filename, int page) {
        ...
        debugDrawer.saveDebugImage(newImage, "removedBlackAreas", filename, page);

        log.info("Extracting lines");
        extractLines(newImage, lines);

        newImage = ImageUtils.createImageWithLines(image.getWidth(), image.getHeight(), lines);
        debugDrawer.saveDebugImage(newImage, "rawLines", filename, page);

        log.info("Combining lines");
        combineLines(lines);

        log.info("Extending lines");
        extendLines(lines, image);

        newImage = ImageUtils.createImageWithLines(image.getWidth(), image.getHeight(), lines);
        debugDrawer.saveDebugImage(newImage, "lines", filename, page);
        return lines;
    }
     ...
     */
    private void extractLines(BufferedImage image, List<Line2D> lines) {
        final int n = 50;
    ...
    private void combineLines(List<Line2D> lines) {
    ...
    private void extendLines(List<Line2D> lines, BufferedImage img) {
        for (Line2D line : lines) {
            if (ImageUtils.isHorizontal(line)) {
                if (line.getX1() > line.getX2())
                    line.setLine(line.getX2(), line.getY1(), line.getX1(), line.getY2());
                extendHorizontalLine(line, lines, img);
            } else {
                if (line.getY1() > line.getY2())
                    line.setLine(line.getX1(), line.getY2(), line.getX2(), line.getY1());
                extendVerticalLine(line, lines, img);
            }
        }
    }

    private void extendHorizontalLine(Line2D line, List<Line2D> lines, BufferedImage img) {
        BufferedImage image = ImageUtils.createImageWithLines(img.getWidth(), img.getHeight(), lines);
    ...
    private void extendVerticalLine(Line2D line, List<Line2D> lines, BufferedImage img) {
        BufferedImage image = ImageUtils.createImageWithLines(img.getWidth(), img.getHeight(), lines);
