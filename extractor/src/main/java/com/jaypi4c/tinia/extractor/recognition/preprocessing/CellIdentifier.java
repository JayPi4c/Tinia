package com.jaypi4c.tinia.extractor.recognition.preprocessing;

import com.jaypi4c.tinia.extractor.utils.DebugDrawer;
...
public class CellIdentifier {
    ...
    public List<Rectangle2D> execute(BufferedImage image, List<Line2D> lines, String filename, int page) {
        ...
        byte[][] nodeMatrix = labelIntersections(image, intersections, filename, page);

        log.info("Searching for cells");
        identifyCells(intersections, cells, nodeMatrix);

        log.info("Found {} cells", cells.size());
        drawCells(image, cells, filename, page);

        return cells;
    }

    private void drawCells(BufferedImage img, List<Rectangle2D> cells, String filename, int page) {

        BufferedImage image = ImageUtils.deepCopy(img);
        ...
        debugDrawer.saveDebugImage(image, "cells", filename, page);
    }

    /**
     * Checks all the lines that are in the list for intersections and adds them to the intersections list.
     */
    private void findIntersections(List<Line2D> lines, List<Point2D> intersections) {
     ...
     * TODO: Link paper
     */
    private byte[][] labelIntersections(BufferedImage img, List<Point2D> intersections, String filename, int page) {
        byte[][] nodeMatrix = new byte[img.getWidth()][img.getHeight()];
        ...
        for (Point2D intersection : intersections) {
            ...
            if (ImageUtils.checkAreaAroundForBlackPixel(img, pointAbove, 3)) {
                b = (byte) (b | 0b0001);
            }
            if (ImageUtils.checkAreaAroundForBlackPixel(img, pointBelow, 3)) {
                b = (byte) (b | 0b0010);
            }
            if (ImageUtils.checkAreaAroundForBlackPixel(img, pointLeft, 3)) {
                b = (byte) (b | 0b0100);
            }
            if (ImageUtils.checkAreaAroundForBlackPixel(img, pointRight, 3)) {
                b = (byte) (b | 0b1000);
            }
        ...
        BufferedImage debugImage = ImageUtils.deepCopy(img);
        ...
        debugDrawer.saveDebugImage(debugImage, "labels", filename, page);

        log.debug("Found {} intersections", intersections.size());
        return nodeMatrix;
    }

    private void identifyCells(List<Point2D> intersections, List<Rectangle2D> cells, byte[][] nodeMatrix) {
        // Find the corresponding Cell for each intersection
        for (Point2D intersection : intersections) {
            Optional<Rectangle2D> cellOpt = findCellForTopLeftCorner(intersection, nodeMatrix, intersections);
            cellOpt.ifPresent(cells::add);
            cellOpt = findCellForBottomLeftCorner(intersection, nodeMatrix, intersections);
            cellOpt.ifPresent(cells::add);
        }
        removeDuplicates(cells);
    }

    private void removeDuplicates(List<Rectangle2D> cells) {
     ...
     */
    private Optional<Rectangle2D> findCellForTopLeftCorner(Point2D intersection, byte[][] nodeMatrix, List<Point2D> intersections) {
     ...
     * @param intersection the intersection to find the cell for
     */
    private Optional<Rectangle2D> findCellForBottomLeftCorner(Point2D intersection, byte[][] nodeMatrix, List<Point2D> intersections) {
